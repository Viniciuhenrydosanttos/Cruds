...
document.getElementById("form-fornecedor").addEventListener("submit", async (e) => {
  ...
  try {
    ...
    if (!res.ok) {
      ...
      throw new Error(erro.error || "Erro desconhecido ao cadastrar fornecedor");
    }
...
document.getElementById("form-entrada").addEventListener("submit", async (e) => {
  ...
  const data = {
    id_fornecedor: parseInt(document.getElementById("entrada-fornecedor").value),
    id_produto: parseInt(document.getElementById("entrada-produto").value),
    quantidade: parseInt(document.getElementById("entrada-quantidade").value),
    data: document.getElementById("entrada-data").value // formato YYYY-MM-DD
  };

  try {
    await fetch("http://192.168.1.4:3000/entradas", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify(data)
    });
    ...
    e.target.reset();
    carregarEntradas(); // atualiza tabela
  } catch (err) {
